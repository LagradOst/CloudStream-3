...
class CimaNowProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        return if (isMovie) {
            ...
            val episodes = ArrayList<Episode>()
                doc.select("ul#eps li").map { episode ->
                    episodes.add(
                        Episode(
    ...
    override suspend fun loadLinks(
        ...
        app.get("$data").document.select("ul#download [aria-label=\"quality\"]").mapNotNull {
            val name = if(it.select("span").text().contains("فائق السرعة")) "Fast Servers" else "Servers"
            it.select("a").map { media ->
                callback.invoke(
                    ExtractorLink(
                        source = this.name,
                        name = name,
                        ...
                        quality = media.text().getIntFromText()!!
                    )
                )
            }
        }
