...
class EstrenosDoramasProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val description = try {
            doc.selectFirst("div.post div.highlight div.font").text()?.substringAfter("Sinopsis:")?.trim()
        } catch (e:Exception){
            null
        }
        ...
        return when (val type = if (episodes.isEmpty()) TvType.Movie else TvType.AsianDrama) {
            TvType.AsianDrama -> {
                return newAnimeLoadResponse(cleanTitle(title), url, type) {
                    japName = null
                    engName = cleanTitle(title)
                    posterUrl = poster
